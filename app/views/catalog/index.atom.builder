...
xml.feed("xmlns" => "http://www.w3.org/2005/Atom",
  ...
  @document_list.each_with_index do |doc, document_counter|
    xml.entry do
      ...
      xml.link    "rel" => "alternate", "type" => "text/html", "href" => polymorphic_url([:curation_concern, doc])
      # add other doc-specific formats, atom only lets us have one per
      # content type, so the first one in the list wins.
      #xml << render_link_rel_alternates([:curation_concern, doc], :unique => true)
      
      xml.id      polymorphic_url([:curation_concern, doc])
